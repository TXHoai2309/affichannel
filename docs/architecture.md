...
Voice 19D, Organic UX 19E.1 và 19E.2 E2E/manual final acceptance đã PASS;
AFF-US-019 đã DONE/ACCEPTED. Xem
...
### AFF-US-020 Shared Media Library contract (20A/20B accepted)
...
không bị rename/backfill/evolve trong 20A/20B. VoiceSegment/VoiceAudioStorage
tiếp tục là domain riêng.

V1 dùng `MediaAssetStorage` riêng, local deterministic và private R2 qua một
server-owned interface, với flow prepare → signed/opaque upload → finalize, và
status `pending_upload/validating/ready/failed/archived`. Phase 20B đã triển khai
additive persistence, repository lifecycle, validation, env mới, và storage
adapters; không lưu binary trong database, không public permanent URL, không nhận
SVG hoặc arbitrary URL import. Protected endpoint, UI activation, and project
cutover vẫn deferred sang 20C–20E.
