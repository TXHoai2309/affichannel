...
export const scriptGeneration = pgTable(
	...
	projectId: text("project_id")
		...
		validSections: text("valid_sections").array().notNull().default(sql`ARRAY[]::text[]`),
		invalidSections: text("invalid_sections").array().notNull().default(sql`ARRAY[]::text[]`),
	...
	(table) => [
		check("script_generation_status_check", sql`${table.status} in (${sql.raw(scriptGenerationStatuses.map((value) => `'${value}'`).join(", "))})`),
		check("script_generation_mode_check", sql`${table.mode} in (${sql.raw(scriptGenerationModes.map((value) => `'${value}'`).join(", "))})`),
		check("script_generation_request_hash_check", sql`${table.requestHash} ~ '^[0-9a-f]{64}$'`),
		check("script_generation_input_hash_check", sql`${table.inputHash} ~ '^[0-9a-f]{64}$'`),
		check("script_generation_prompt_hash_check", sql`${table.promptHash} ~ '^[0-9a-f]{64}$'`),
		check("script_generation_idempotency_key_check", sql`length(trim(${table.idempotencyKey})) between 8 and 200`),
		check("script_generation_mode_parent_check", sql`(${table.mode} = 'full' and ${table.parentGenerationId} is null) or (${table.mode} = 'repair' and ${table.parentGenerationId} is not null)`),
		...
		check("script_generation_token_check", sql`${table.inputTokens} is null or ${table.inputTokens} >= 0`),
		check("script_generation_output_token_check", sql`${table.outputTokens} is null or ${table.outputTokens} >= 0`),
		check("script_generation_estimated_cost_check", sql`${table.estimatedCostMicros} is null or ${table.estimatedCostMicros} >= 0`),
		check("script_generation_actual_cost_check", sql`${table.actualCostMicros} is null or ${table.actualCostMicros} >= 0`),
		check("script_generation_cost_currency_check", sql`((${table.estimatedCostMicros} is null and ${table.actualCostMicros} is null and ${table.currency} is null) or ((${table.estimatedCostMicros} is not null or ${table.actualCostMicros} is not null) and ${table.currency} ~ '^[A-Z]{3}$'))`),
		uniqueIndex("script_generation_idempotency_unique").on(table.workspaceId, table.idempotencyKey),
		unique("script_generation_scope_id_unique").on(table.workspaceId, table.projectId, table.id),
		uniqueIndex("script_generation_pending_project_unique").on(table.workspaceId, table.projectId).where(sql`${table.status} = 'pending'`),
		index("script_generation_latest_idx").on(table.workspaceId, table.projectId, table.createdAt, table.id),
		index("script_generation_parent_idx").on(table.workspaceId, table.parentGenerationId),
